...
\begin{itemize}
     ...
     \item Distributed system aspects are out of this work.
     \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item By the pigeon houle principle, the injectivity of the hash is impossible.
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item It is not possible to know the private key from the public key and signature
      generated by it.
    \item It is possible to know if a signature and a public key coincide to the same
      private key that generate both of them.
...
\begin{frame}
  ...
  \begin{itemize}
    \item Hash injective is really impossible to proof.
      But we postulate it because the probability of hash collision is very low.
    \item In my master thesis, I postulate each function of crypto postulates.
      However, it is better to put it as a variable.
      So, the code can be more general for every crypto function that have these properties.
  \end{itemize}
\end{frame}
...
\begin{frame}
   ...
   \begin{itemize}
     ...
     \item In Bitcoin, there are two kind of transactions:
       the coinbase transactions and the normal transaction.
       There is just one coinbase transaction per block and it just has outputs.
       While the normal transaction have inputs and outputs.
       The inputs of transaction should be outputs of other transactions that have never been used
       as input of one transaction.
    \item Furthermore, each transaction must have a signature generated from the private key proving
...
\begin{frame}
   ...
   \begin{itemize}
     ...
     \item In this way, it is possible to fingerprint the information of all transactions
