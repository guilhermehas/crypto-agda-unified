...
\begin{frame}
  \frametitle{Postulates}
\begin{code}
record WithCryptoPostulates (cryptoSets : CryptoSets)
  : Set₁ where
  open CryptoSets cryptoSets public
  field
...
record CryptoPostulates : Set₁ where
  field
    ...
    cryptoPostulates : WithCryptoPostulates cryptoSets
...
\begin{frame}
  \frametitle{Ledger}
\begin{code}
record Ledger (size : ℕ) (totalAmount : Amount) : Set where
  constructor ledgerC
  field
...
record LedgerViewWithAmount (size : ℕ) (totalAmount : Amount)
  (p q : Fin size) (p≢q : p ≢ q) (amount : Amount) : Set where
\end{code}
\AgdaHide{
\begin{code}
