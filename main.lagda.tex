...
\begin{itemize}
  \item In Satoshi Nakamoto's vision, Bitcoin would be minerated by everyone.
    But nowadays, there are just a few miners who mined it in few places on Earth.
  \item Because of proof of work of Bitcoin, mining spends a lot of energy.
    However, most of the energy to mine is green. It is usually geothermic energy that would not be used where it was produced.
  \item Some other cryptocurrencies, such as Cardano, use Proof of Stake to solve this problem. In addition, mining rewards go to people who have a Proof of Stake cryptocurrency.
\end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \item In my master thesis, I postulate each function of crypto postulates.
      ...
      So, the code can be more general for every crypto function that has these properties.
  \end{itemize}
\end{frame}
...
\begin{frame}
   ...
   \begin{itemize}
     ...
     \item In Bitcoin, there is two kinds of transactions:
...
\begin{frame}
   ...
   \begin{itemize}
     ...
     \item There is a limit of transactions that each block can have (in Bitcoin, it's 10 MB of data).
       Therefore, it is important to pay a transaction fee for this transaction to be included in the blockchain.
...
\begin{frame}{More complex parts to add}
  \begin{itemize}
    ...
    \item Specify the protocol between client and node and between node to node.
      However, the hardest part has already been coded, which is adding the evidence to the raw data that the client sends.
